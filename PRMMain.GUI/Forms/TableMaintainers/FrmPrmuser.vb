...
#Region "#### Public & Private Global Objects"
    ...
    Public FormSelectedObject As New PrmuserBO
...
#Region "### Maintainer Initializer"
    ...
    Private Sub FrmPrmuser_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        totalRecords = PrmuserBLL.GetTotalRecords(False)
...
#Region "### Add, Copy, Refresh, Print, Edit and Delete Buttons Click Events, Functions and Subs"
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        Dim Form01 As New FrmPrmuser01
        Form01.rowAdding = True
        Dim objectBO As New PrmuserBO
    ...
    Private Sub gridTemplate_myGridCellClick(sender As Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles grid.myGridCellClick
		...
		objectBO.prmuser = thisRow.Item("prmuser").ToString.Trim
			...
			'
			thisRow.Item("prmuser") = If(objectBO.prmuser Is Nothing, "", objectBO.prmuser.Trim)
        ...
        If columnName.Trim = "delButton" And delCell.Enabled Then
            ...
            If MessageBox.Show("Are you sure you wish to delete User: " & vbCrLf  & thisRow.Item("Prmuser").ToString.Trim  & " ?",
		...
		'
            ...
            Try
                ...
                Dim deleted As Boolean = PrmuserBLL.Delete(objectBO)
            ...
            Catch ex As Exception
                ...
                If _message.Trim.Length > 6 Then
                    ...
                    If _message.Substring(0, 7) = "SQL0532" Then
                        ...
                        _message = _message & vbCrLf & "User: "  & thisRow.Item("Prmuser").ToString.Trim  & " is used in the above mentioned table" & vbCrLf & "Would You like to mark this code inactive?"
		...
		'
                    objectBO = PrmuserBLL.GetUnique(objectBO)
                    objectBO.active = "F"
                    Dim updated As Boolean = PrmuserBLL.Update(objectBO)
                    '
			'Code to Get the thisRow.Items Columns for Main Grid.
			'
			thisRow.Item("prmuser") = If(objectBO.prmuser Is Nothing, "", objectBO.prmuser.Trim)
...
#Region "### Getting and Filtering Main Grid Local Data"
    ...
    Private Sub RefreshMessages()
		...
		'
		If Not String.IsNullOrEmpty(txtPrmuser.Text.Trim()) Then
			filter.Append($"User Id(Start with: '{txtPrmuser.Text.Trim()}')")
		End If
		'
		'End Code.
		'
    ...
    Private Sub TextBoxes_TextChanged(sender As Object, e As EventArgs) Handles txtPrmuser.TextChanged
    ...
    Private Sub txt_KeyDown(sender As Object, e As KeyEventArgs) Handles txtPrmuser.KeyDown
...
#Region "### Getting and Filtering Main Grid Remote Data"
    ...
    Private Function GetFilterStringForRemoteData() As String
		...
		'
		If Not String.IsNullOrEmpty(txtPrmuser.Text.Trim()) Then
			filter.Append($"upper(prmuser) like '{txtPrmuser.Text.Trim().ToUpper()}%'")
		End If
		'
		'End Code.
		'
    ...
    Private Sub bgwLoadData_DoWork(ByVal sender As Object, ByVal e As DoWorkEventArgs) Handles bgwLoadData.DoWork
        ...
        sqlQuery = $"select 
							...
							crsfile.Prmuser.descr as descr,
							crsfile.Prmuser.active as active                            
                         from
	                        crsfile.Prmuser
                                
                         where {If(Not ShowAllRecors, " crsfile.Prmuser.active = 'T' ", " crsfile.Prmuser.active in ('T','F') ")}
                            {RemoteFilterString}"

        'Debug.Write(sqlQuery)
        Try
            table = PrmuserBLL.GetTable(sqlQuery)
        Catch ex As Exception
...
#Region "### Resize Main Grid Columns"
    ...
    Private Sub FrmPrmuser_ResizeEnd(sender As Object, e As EventArgs) Handles MyBase.ResizeEnd
...
#Region "### Window Short Cuts"
    ...
    Private Sub FrmPrmuser_KeyDown(sender As Object, e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub cbcActiveRecrods_DropDownItemClicked(sender As Object, e As ToolStripItemClickedEventArgs) Handles cbcActiveRecrods.DropDownItemClicked
        ...
        Select Case e.ClickedItem.Tag
            ...
            Case "noactive"
                ...
                totalRecords = PrmuserBLL.GetTotalRecords(True)
